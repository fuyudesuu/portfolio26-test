        ...
        <div className="flex gap-2 mb-8 flex-wrap">
          {FILTERS.map((f) => (
            <button key={f.key} onClick={() => { setFilter(f.key); setExpanded(null); }}
              className={`px-4 py-2 rounded-full text-[13px] font-medium cursor-pointer outline-none border-[1.5px] transition-all duration-200 font-[inherit] ${filter === f.key ? "bg-[var(--accent-1)] text-white border-[var(--accent-1)]" : "bg-transparent text-[var(--fg-2)] border-[var(--border)] hover:border-[var(--fg-3)]"}`}>
              {f.label}
            </button>
          ))}
        </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className={"flex-1 min-w-0 bg-[var(--bg-card)] border border-[var(--border)] rounded-card p-7 pl-8 cursor-pointer transition-all duration-300 " + (isOpen ? "shadow-[0_8px_32px_var(--shadow)]" : "hover:shadow-[0_6px_24px_var(--shadow)]")}
                      ...
                      <AnimatePresence>
                        {isOpen && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}>
                            ...
                            <p className="text-sm leading-relaxed text-[var(--fg-2)]">{ev.content}</p>
                            {ev.tags.length > 0 && (
                              <div className="flex gap-2 flex-wrap mt-3.5">
                                {ev.tags.map((t) => (<span key={t} className="px-3 py-1 rounded-full text-xs font-medium bg-[var(--bg-2)] text-[var(--fg-2)] border border-[var(--border)]">{t}</span>))}
                              </div>
                            )}
                          </motion.div>
                        )}
                      ...
                      <div className="flex items-center justify-between mt-3.5 pt-2">
                        <span className="text-[13px] text-[var(--fg-3)]">{isOpen ? "Click to collapse" : "Click to expand"}</span>
